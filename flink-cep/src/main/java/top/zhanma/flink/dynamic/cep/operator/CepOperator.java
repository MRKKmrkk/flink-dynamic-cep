...
package top.zhanma.flink.dynamic.cep.operator;
...
import top.zhanma.flink.dynamic.cep.EventComparator;
import top.zhanma.flink.dynamic.cep.functions.PatternProcessFunction;
import top.zhanma.flink.dynamic.cep.functions.TimedOutPartialMatchHandler;
import top.zhanma.flink.dynamic.cep.nfa.ComputationState;
import top.zhanma.flink.dynamic.cep.nfa.NFA;
import top.zhanma.flink.dynamic.cep.nfa.NFAState;
import top.zhanma.flink.dynamic.cep.nfa.NFAStateSerializer;
import top.zhanma.flink.dynamic.cep.nfa.State;
...
public class CepOperator<IN, KEY, OUT>
    ...
    private boolean needChange(IN element){
        ...
        return userFunction.getFlagNeedListener() && userFunction.isPatternChanged(element);
    }

    /**
     * Modifications Copyright [2023/07/07] [MRKKmrkk]
     * 触发新逻辑注入时通过调用用户代码得到返回的pattern更新NFA
     */
...
//		清理以前的未完成的数据，以及共享缓存
    ...
    /**
     * Modifications Copyright [2023/07/07] [MRKKmrkk]
     * 用于清理以前未匹配完成的部分状态，并且用新逻辑初始化新的NFAstate
