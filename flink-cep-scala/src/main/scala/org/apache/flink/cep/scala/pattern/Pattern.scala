...
import top.zhanma.flink.dynamic.cep
import top.zhanma.flink.dynamic.cep.nfa.aftermatch.AfterMatchSkipStrategy
import top.zhanma.flink.dynamic.cep.pattern.conditions.IterativeCondition.{Context => JContext}
import top.zhanma.flink.dynamic.cep.pattern.conditions.{IterativeCondition, SimpleCondition}
import top.zhanma.flink.dynamic.cep.pattern.{MalformedPatternException, Quantifier, Pattern => JPattern}
...
class Pattern[T , F <: T](jPattern: JPattern[T, F]) {
  ...
  def where(condition: (F, Context[F]) => Boolean): Pattern[T, F] = {
    val condFun = new IterativeCondition[F] {
      val cleanCond = org.apache.flink.cep.scala.cleanClosure(condition)
  ...
  def until(untilCondition: (F, Context[F]) => Boolean): Pattern[T, F] = {
    val condFun = new IterativeCondition[F] {
      val cleanCond = org.apache.flink.cep.scala.cleanClosure(untilCondition)
